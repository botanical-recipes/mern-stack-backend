...
const { ItemModel, UserModel } = require("./utils/schemqModels");
...
app.get("/item/:id", async (req, res) => {
  try {
    ...
    return res.status(200).json({ nessage: "アイテム読み取り成功(シングル)", singleItem: singleItem });
  } catch {
...
app.delete("/item/delete/:id", auth, async (req, res) => {
  try {
    ...
    res.status(400).json({ mesaage: "アイテム削除失敗" });
  }
});
